...
politicsBtn.addEventListener("click", () => {
  const politicPost = data.filter((post) => post.genre == "politics");
  document.getElementById("app").innerHTML = "";
  renderMain(politicPost);
});

const sportsBtn = document.getElementById("sports-btn");
sportsBtn.addEventListener("click", () => {
  const sportPost = data.filter((post) => post.genre == "sports");
  document.getElementById("app").innerHTML = "";
  renderMain(sportPost);
});

const todayBtn = document.getElementById("today-btn");
todayBtn.addEventListener("click", () => {
  const todayPost = data.filter((post) => post.date == "2025-08-26");
  document.getElementById("app").innerHTML = "";
  renderMain(todayPost);
});

const yesterdayBtn = document.getElementById("yesterday-btn");
yesterdayBtn.addEventListener("click", () => {
  const yesterdayPost = data.filter((post) => post.date == "2025-08-25");
  document.getElementById("app").innerHTML = "";
  renderMain(yesterdayPost);
});

function renderMain(postObjects) {
  postObjects.map((post) => {
    ...
    const postImageSec = document.createElement("div");
    postImageSec.setAttribute("class", "postImageSec");
    ...
    postDiv.appendChild(postImageSec);
    postDiv.appendChild(postDetails);

    postImageSec.appendChild(image);
    postDetails.appendChild(title);
    postDetails.appendChild(text);

    document.getElementById("app").appendChild(posts);
  });
}
